...
mod tests {
    ...
    #[tokio::test]
    async fn alternate_screen_keeps_scrollbar_gutter_stable_but_hides_the_bar() {
        let mut app = AppState::test_new();
        let mut workspace = Workspace::test_new("test");
        let root_pane = workspace.tabs[0].root_pane;
        workspace.tabs[0].runtimes.insert(
        ...
        // The gutter column width stays reserved (39) across alt-screen
        // enter/exit -- only the scrollbar's visibility toggles, since
        // resizing the PTY on every alt-screen transition can wrap an
        // already-drawn prompt row. See `terminal_inner_rect`.
        assert_geometry(39, true);
        app.workspaces[0].tabs[0].runtimes[&root_pane].test_process_pty_bytes(b"\x1b[?1049h");
        assert_geometry(39, false);
