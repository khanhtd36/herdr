...
impl PaneRuntimeIo {
    ...
    /// Waits for the freshly spawned shell to actually produce output and
    /// settle (rather than a fixed delay) before writing. A slow shell
    /// startup (e.g. a PowerShell profile loading cold after a machine
    /// reboot) can easily exceed a fixed delay, which would otherwise cause
    /// the write to land before the shell is reading stdin and get silently
    /// dropped.
        ...
        match self {
            PaneRuntimeIo::Actor(actor) => {
                ...
                tokio::spawn(async move {
                    ...
                    .await;
                    if let Err(err) = actor.try_write_user_input(bytes) {
...
impl PaneRuntime {
    ...
    // Production code reads alt-screen state via `self.terminal` directly
    // (e.g. `handoff_history_ansi`); this wrapper only exists for test
    // assertions (see `pane_clear_screen_in_alt_screen_never_writes_to_pty`).
