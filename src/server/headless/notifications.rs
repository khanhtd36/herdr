...
impl HeadlessServer {
    ...
    pub(super) fn forward_semantic_agent_transition(
        ...
        let (semantic_kind, event_text, sound) = match kind {
            ...
            // Never produced by `notification_toast_for_state_change_with_agent_labels`,
            // which only derives agent-transition kinds.
            crate::app::state::ToastKind::Warning => return false,
        };
