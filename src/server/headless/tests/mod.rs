...
async fn first_shell_surface_resizes_a_pane_that_entered_alternate_screen() {
    ...
    // The gutter reservation stays stable across alternate-screen transitions
    // (see `terminal_inner_rect` in src/ui/panes.rs), so entering the
    // alternate screen does not resize the pane.
    assert_eq!(
...
async fn different_size_shells_receive_geometry_specific_patches_from_one_dirty_collection() {
    ...
    // The gutter reservation stays stable across alt-screen transitions
    // (see `terminal_inner_rect` in src/ui/panes.rs) so the width doesn't
    // change; only the scrollbar's visibility does.
    assert_eq!(
        server.app.state.workspaces[0].test_runtimes[&pane_id].current_size(),
        initial_size
    );
    assert_eq!(
        ...
        large_initial.panes[0].inner_rect.width
    );
    assert_eq!(
        small_alt.panes[0].inner_rect.width,
        small_initial.panes[0].inner_rect.width
    );
