...
use super::state::{
    ...
    PendingPaneSwap, ToastKind, ToastNotification, ToastTarget,
};
...
impl AppState {
    ...
    /// Mark the focused pane for a swap. Always sets the mark (overwrites
    /// any existing one) — mirrors tmux's `select-pane -m`, not a toggle.
    ...
    /// Clear any armed pane mark, regardless of current focus or workspace.
    pub fn unmark_pane(&mut self) {
        self.pending_pane_swap = None;
    }

    /// Swap the marked pane with the focused pane, same tab or another tab
    /// in this workspace. The mark is left armed on every failure except a
    /// dead marked pane (closed/died since marking) — a cross-workspace
    /// attempt shows a warning but keeps the mark so the user can switch
    /// back and retry; same-pane and no-op cases are silent, matching the
    /// existing directional-swap no-op precedent.
