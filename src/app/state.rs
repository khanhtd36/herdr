...
pub struct AppState {
    ...
    /// press on a different pane to swap into. Not persisted, not synced
    /// across clients, cleared on restart. Survives switching to a different
    /// workspace so the swap can still be confirmed (or cleared silently)
    /// later; it holds the source workspace so a cross-workspace confirm
    /// attempt can be recognized and rejected with a warning instead of
    ...
    /// Effective `[detect] enabled` value. When false, agent detection is
    /// fully disabled and the sidebar collapses to a full-width workspace
    /// list (no agent detail panel).
