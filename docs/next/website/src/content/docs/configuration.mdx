...
Only applies when the pane was still running that command (not idle at a shell prompt) at the time of the previous shutdown. Agent-resume (above) always takes priority when both could apply.
