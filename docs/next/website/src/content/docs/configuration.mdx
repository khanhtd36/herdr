...
Split a new tab's starting pane automatically:
...
`default_split = "vertical"` gives every new tab two side-by-side panes; `"horizontal"` stacks them instead. Default is `"none"` (a single pane, matching prior behavior). This applies whenever a tab is created fresh — keybind, CLI, or socket API — and does not apply when restoring a saved session or applying an explicit layout.
...
By default Herdr resumes each Agent with its official CLI binary name (e.g. `claude`, `codex`). Override the binary and/or add flags per Agent with `agent_resume_command`, keyed by Agent id:

```toml
[session.agent_resume_command]
claude = "cld"
codex = "codex --dangerously-bypass-approvals-and-sandbox"
```

The value replaces only the resume binary and any extra tokens before it; Herdr still appends its own resume arguments (e.g. `--resume <session-id>`) after it. Values are split on whitespace, so quoting or arguments containing spaces are not supported. Unknown Agent ids or empty values are ignored, with a config diagnostic.

### Restoring running commands

Herdr can also rerun the foreground command a non-Agent pane was running (e.g. a TUI like `lazygit`) after a server restart, by feeding the exact command line it last ran into a freshly spawned shell. This is off by default:
...
Only applies when the pane was still running that command (not idle at a shell prompt) at the time of the previous shutdown. Agent-resume (above) always takes priority when both could apply. Detecting arbitrary running commands is currently Linux/macOS only; see [Session state and restore](/docs/session-state/) for the Windows limitation.
