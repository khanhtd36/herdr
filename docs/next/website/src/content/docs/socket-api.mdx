...
Swap works within a tab or across tabs in the same workspace; it rejects a
swap across workspaces. It preserves split shape, split ratios, pane ids, and
...
`cross_workspace`. When a tab is zoomed, swap keeps zoom active and mutates the
hidden full-tab layout.
