...
交换可在同一标签页内进行,也可在同一工作区的不同标签页之间进行;跨工作区的交换会被拒绝。它保留分割形状、分割比例、窗格 id 和运行中的进程。响应是 `type: "pane_swap"`,带 `changed`、可选的 `reason`、`source_pane_id`、可选的 `target_pane_id`、`focused_pane_id` 和 `layout`。reason 的取值有 `no_neighbor`、`same_pane`、`not_found` 和 `cross_workspace`。标签页处于缩放状态时,交换保持缩放,并修改隐藏的整页布局。
