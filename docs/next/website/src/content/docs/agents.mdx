...
## Disabling agent detection

Set `[detect] enabled = false` to fully disable AI-agent detection: Herdr stops probing foreground processes and reading pane screen content for agent state. This is a kill switch, not a per-agent setting.

Disabling detection also forces off `[session] resume_agents_on_restore`, `[update] manifest_check`, and `[ui] agent_panel_sort` (treated as `spaces`), since those all depend on detection running. The sidebar's agent detail panel collapses and the workspace list uses the full sidebar width instead.
