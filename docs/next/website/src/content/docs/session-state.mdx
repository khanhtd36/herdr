...
Snapshot restore does not preserve running shells, servers, tests, or arbitrary processes. Panes that cannot use a stronger restore path come back as new shells in their saved directories, unless [restoring running commands](#restoring-running-commands) applies.
...
Herdr can rerun a non-agent pane's foreground command (e.g. a TUI like `lazygit`) after a server restart, by feeding the exact command line it last ran into a freshly spawned shell. This is off by default:
...
It only applies when the pane was still running that command — not idle at a shell prompt — at the time of the previous shutdown. Native agent session restore always takes priority: a pane that qualifies for agent resume never falls through to this path. If the rerun fails (missing binary, saved cwd gone), the pane is left at a plain shell prompt.

This is unrelated to live handoff below: a handed-off pane's process never dies, so there is nothing to rerun.

:::caution[Windows support is limited]
On Windows, detecting which command a pane is running is currently narrowed to recognized AI agents, so this setting has no effect for arbitrary commands (a plain `lazygit` pane, for example) — only Linux and macOS reliably detect and restore arbitrary foreground commands today.
:::
